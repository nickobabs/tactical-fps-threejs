...
var init = async (impl) => {
  ...
  if (impl) {
    ...
    const defaultExport = (await import("./recast-navigation.wasm-compat-SWG226ON.js")).default;
...
var IntArray = class _IntArray extends BaseArray {
  ...
  static fromRaw(raw) {
    return new _IntArray(raw);
  }
};
var UnsignedIntArray = class _UnsignedIntArray extends BaseArray {
  ...
  static fromRaw(raw) {
    return new _UnsignedIntArray(raw);
  }
};
var UnsignedCharArray = class _UnsignedCharArray extends BaseArray {
  ...
  static fromRaw(raw) {
    return new _UnsignedCharArray(raw);
  }
};
var UnsignedShortArray = class _UnsignedShortArray extends BaseArray {
  ...
  static fromRaw(raw) {
    return new _UnsignedShortArray(raw);
  }
};
var FloatArray = class _FloatArray extends BaseArray {
  ...
  static fromRaw(raw) {
    return new _FloatArray(raw);
  }
};
var VerticesArray = FloatArray;
var TrianglesArray = IntArray;
var TriangleAreasArray = UnsignedCharArray;
var ChunkIdsArray = IntArray;
var TileCacheData = UnsignedCharArray;
var vec3 = {
  toRaw: ({
    x,
    y,
...
var DetourOffMeshConnection = class {
  constructor(raw) {
    this.raw = raw;
  }
  pos(index) {
...
var DetourMeshTile = class _DetourMeshTile {
  constructor(raw) {
    this.raw = raw;
  }
  salt() {
  ...
  next() {
    return new _DetourMeshTile(this.raw.next);
  }
};
var createNavMeshData = (navMeshCreateParams) => {
...
var rgbToDuRgba = (hexColor) => {
  const r = hexColor >> 16 & 255;
  const g = hexColor >> 8 & 255;
  const b = hexColor & 255;
  return r | g << 8 | b << 16 | 255 << 24;
};
var NavMeshGetTilesAtResult = class {
  constructor(raw) {
    this.raw = raw;
  }
  tiles(index) {
...
var NavMeshRemoveTileResult = class {
  constructor(raw) {
    this.raw = raw;
  }
  data() {
...
var NavMeshCalcTileLocResult = class {
  constructor(raw) {
    this.raw = raw;
  }
  tileX() {
...
var NavMeshStoreTileStateResult = class {
  constructor(raw) {
    this.raw = raw;
  }
  data() {
...
var NavMeshParams = class _NavMeshParams {
  constructor(raw) {
    this.raw = raw;
  }
  static create(params) {
    ...
    return new _NavMeshParams(raw);
  }
  clone() {
    return _NavMeshParams.create({
      orig: {
...
var getNavMeshPositionsAndIndices = (navMesh, flags) => {
  const positions = [];
  const indices = [];
  let tri = 0;
...
var floodFillPruneNavMesh = (navMesh, startPolyRefs) => {
...
var setRandomSeed = (seed) => {
  Raw.Module.FastRand.prototype.setSeed(seed);
};
var recastConfigDefaults = {
  borderSize: 0,
  tileSize: 0,
  cs: 0.2,
  ch: 0.2,
...
var createRcConfig = (partialConfig) => {
  const config = {
...
var RecastSpan = class _RecastSpan {
  constructor(raw) {
    this.raw = raw;
  }
  smin() {
  ...
  next() {
    return !Raw.isNull(this.raw.next) ? new _RecastSpan(this.raw.next) : null;
  }
};
var RecastSpanPool = class _RecastSpanPool {
  constructor(raw) {
    this.raw = raw;
  }
  next() {
    return !Raw.isNull(this.raw.next) ? new _RecastSpanPool(this.raw.next) : null;
  }
  items(index) {
    return new RecastSpan(this.raw.get_items(index));
  }
};
var RecastHeightfield = class {
  constructor(raw) {
    this.raw = raw;
  }
  width() {
...
var RecastCompactHeightfield = class {
  constructor(raw) {
    this.raw = raw;
  }
  width() {
...
var RecastHeightfieldLayer = class {
  constructor(raw) {
    this.raw = raw;
  }
  bmin() {
...
var RecastHeightfieldLayerSet = class {
  constructor(raw) {
    this.raw = raw;
  }
  layers(index) {
...
var RecastPolyMeshDetail = class {
  constructor(raw) {
    this.raw = raw;
  }
  meshes(index) {
...
var filterLowHangingWalkableObstacles = (buildContext, walkableClimb, heightfield) => {
  return Raw.Recast.filterLowHangingWalkableObstacles(buildContext.raw, walkableClimb, heightfield.raw);
};
var filterLedgeSpans = (buildContext, walkableHeight, walkableClimb, heightfield) => {
  return Raw.Recast.filterLedgeSpans(buildContext.raw, walkableHeight, walkableClimb, heightfield.raw);
};
var filterWalkableLowHeightSpans = (buildContext, walkableHeight, heightfield) => {
  return Raw.Recast.filterWalkableLowHeightSpans(buildContext.raw, walkableHeight, heightfield.raw);
};
var getHeightFieldSpanCount = (buildContext, heightfield) => {
  return Raw.Recast.getHeightFieldSpanCount(buildContext.raw, heightfield.raw);
};
var buildCompactHeightfield = (buildContext, walkableHeight, walkableClimb, heightfield, compactHeightfield) => {
...
var buildContours = (buildContext, compactHeightfield, maxError, maxEdgeLen, contourSet, buildFlags = Recast.RC_CONTOUR_TESS_WALL_EDGES) => {
  return Raw.Recast.buildContours(buildContext.raw, compactHeightfield.raw, maxError, maxEdgeLen, contourSet.raw, buildFlags);
};
var buildPolyMesh = (buildContext, contourSet, nvp, polyMesh) => {
  return Raw.Recast.buildPolyMesh(buildContext.raw, contourSet.raw, nvp, polyMesh.raw);
};
var mergePolyMeshes = (buildContext, meshes, outPolyMesh) => {
  return Raw.Recast.mergePolyMeshes(buildContext.raw, meshes.map((m) => m.raw), meshes.length, outPolyMesh.raw);
};
var buildPolyMeshDetail = (buildContext, mesh, compactHeightfield, sampleDist, sampleMaxError, polyMeshDetail) => {
...
var exportNavMesh = (navMesh) => {
  return exportImpl(navMesh);
};
var exportTileCache = (navMesh, tileCache) => {
  return exportImpl(navMesh, tileCache);
};
var DetourTileCacheParams = class _DetourTileCacheParams {
  constructor(raw) {
    this.raw = raw;
  }
  static create(config) {
    ...
    return new _DetourTileCacheParams(tileCacheParams);
  }
};
var TileCache = class {
...
var TileCacheMeshProcess = class {
  constructor(process) {
...
var importTileCache = (data, tileCacheMeshProcess) => {
  const {
    navMeshExport,
    dataHeap
  } = createNavMeshExport(data);
...
//# sourceMappingURL=chunk-MPBQPLVL.js.map
